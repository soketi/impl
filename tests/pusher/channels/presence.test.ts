...
describe('pusher/channels/presence', () => {
    test('join and leave', () => new Promise<void>(async (done) => {
        ...
        await conns.subscribeToChannel(conn, {
            ...
            data: {
                channel: 'presence-1',
                auth: app.key + ':' + (await app.createToken(`${conn.id}:presence-1:${JSON.stringify(user)}`)),
                channel_data: JSON.stringify(user),
            },
        });

        expect([...conn.subscribedChannels]).toEqual(['presence-1']);
        expect(conn.presence).toEqual(new Map([
            ['presence-1', { user_id: '1', user_info: { name: 'John Doe' } }],
        ]));
        expect(conns.channels.get('presence-1')).lengthOf(1);

        await conns.unsubscribeFromChannel(conn, 'presence-1');

        expect([...conn.subscribedChannels]).toEqual([]);
        expect(conn.presence).toEqual(new Map());
        expect(conns.channels.get('presence-1')).toBeUndefined();
    }));
    ...
    test('connect but get unauthorized', async () => new Promise<void>(async (done) => {
        ...
        const conn = new PusherConnection('test', {
            send: (message) => {
                ...
                expect(data.channel).toEqual('presence-1');
