...
describe('pusher/channels/private', () => {
    test('join and leave', () => new Promise<void>(async (done) => {
        ...
        await conns.subscribeToChannel(conn, {
            ...
            data: {
                channel: 'private-1',
                auth: app.key + ':' + (await app.createToken(`${conn.id}:private-1`)),
            },
        });

        expect([...conn.subscribedChannels]).toEqual(['private-1']);
        expect(conn.presence).toEqual(new Map());
        expect(conns.channels.get('private-1')).lengthOf(1);

        await conns.unsubscribeFromChannel(conn, 'private-1');

        expect([...conn.subscribedChannels]).toEqual([]);
        expect(conn.presence).toEqual(new Map());
        expect(conns.channels.get('private-1')).toBeUndefined();
    }));
    ...
    test('connect but get unauthorized', async () => new Promise<void>(async (done) => {
        ...
        const conn = new PusherConnection('test', {
            send: (message) => {
                ...
                expect(data.channel).toEqual('private-1');
